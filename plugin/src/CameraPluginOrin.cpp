...
public:
    CameraPluginOrinImpl(const CameraPluginParams &params)
        : params_{params}, appCfg_{std::make_shared<CAppCfg>()} {

          };

    ~CameraPluginOrinImpl(void) {
        close();
        thread_.join();
    };
    ...
    int8_t startStream(uint32_t cameraId) {
        mask_ |= 1 << cameraId;
        appCfg_->SetMask(mask_);
        return 0;
    }

    int8_t stopStream(uint32_t cameraId) {
        mask_ |= ~(1 << cameraId);
        appCfg_->SetMask(mask_);
        return 0;
    }
...
CameraPluginOrin::CameraPluginOrin(const CameraPluginParams &params)
    : impl_{std::make_unique<CameraPluginOrinImpl>(params)} {}

CameraPluginOrin::~CameraPluginOrin(void) {}

int8_t CameraPluginOrin::loadConfig(const std::string &filename) { return impl_->loadConfig(filename); }

int8_t CameraPluginOrin::open(void) { return impl_->open(); }

int8_t CameraPluginOrin::close(void) { return impl_->close(); }

int8_t CameraPluginOrin::open(uint32_t cameraId) { return impl_->open(cameraId); }

int8_t CameraPluginOrin::close(uint32_t cameraId) { return impl_->close(cameraId); }

int8_t CameraPluginOrin::registerStatusCallback(CameraEventCallback function) {
    return impl_->registerStatusCallback(function);
}

int8_t CameraPluginOrin::startStream(uint32_t cameraId) { return impl_->startStream(cameraId); }

int8_t CameraPluginOrin::stopStream(uint32_t cameraId) { return impl_->stopStream(cameraId); }

int8_t CameraPluginOrin::registerStreamCallback(uint32_t cameraId, CameraCallback function) {
