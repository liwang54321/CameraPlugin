...
#ifndef CIPCLINKQNX_HPP
#define CIPCLINKQNX_HPP
...
constexpr uint32_t kWriteDefaultTimeoutMs = 1000UL;
constexpr uint32_t kEventWaitInfinite = UINT32_MAX;
...
class CIpcLink : public std::enable_shared_from_this<CIpcLink>
{
  ...
  private:
    void SendConnectMsg();
    NvError WaitEvent(uint32_t value, uint32_t uTimeoutMs);
    void ReadThreadFunc();
